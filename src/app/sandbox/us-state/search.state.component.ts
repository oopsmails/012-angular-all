...
export class SearchStateComponent implements OnInit {
  ...
  constructor(
    ...
    private sandboxDataService: SandboxDataService
  ) {}
  ...
  private tokenizeSearchPredicate(item: UsState, searchTextTokens: string[]): boolean {
    let result = true;
    searchTextTokens.forEach((searchToken: string) => {
      result =
        result && item.stateName.toLocaleLowerCase().includes(searchToken.toLocaleLowerCase());
    });
    return result;
  }
}
